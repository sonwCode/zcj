...
class BrowserRegistrationFlow:
    ...
    def run(self, ctx: RegistrationContext) -> RegistrationResult:
        ...
        identity_provider = getattr(ctx.identity, "identity_provider", "")
        is_phone_registration = identity_provider == "phone"

        if self.adapter.capability.browser_mailbox_requires_email and not is_phone_registration:
            ensure_identity_email(ctx, f"{ctx.platform_display_name} 浏览器模式需要邮箱地址")
        if self.adapter.capability.browser_mailbox_requires_mailbox and not is_phone_registration:
        ...
        if self.adapter.otp_spec and not is_phone_registration:
        ...
        if is_phone_registration and artifacts.phone_callback is None:
            raise RuntimeError("手机号注册需要配置可用的接码平台和 API Key")

        try:
            ctx.log(f"{ctx.platform_display_name} 浏览器注册：准备创建 worker")
            ...
            ctx.log(f"{ctx.platform_display_name} 浏览器注册：worker 创建完成，准备调用 runner")
            raw = self.adapter.browser_register_runner(worker, ctx, artifacts)
            ctx.log(f"{ctx.platform_display_name} 浏览器注册：runner 返回，准备映射结果")
...
class ProtocolPhoneFlow:
    ...
    def run(self, ctx: RegistrationContext) -> RegistrationResult:
        ...
        if artifacts.phone_callback is None:
            raise RuntimeError("手机号协议注册需要配置可用的接码平台和 API Key")
        try:
            worker = self.adapter.worker_builder(ctx, artifacts)
            raw = self.adapter.register_runner(worker, ctx, artifacts)
            artifacts.raw_result = raw

...
def _create_local_ms_pool(extra: dict, proxy: str | None) -> 'BaseMailbox':
    ...
    return LocalMicrosoftMailboxPool(
        ...
        plus_alias_enabled=str(
            extra.get("mailbox_alias_enabled", extra.get("gmail_alias_enabled", ""))
        ).strip().lower() in {"1", "true", "yes", "on"},
        plus_alias_count=extra.get(
            "mailbox_alias_count",
...
def create_mailbox(provider: str, extra: dict = None, proxy: str = None) -> 'BaseMailbox':
    ...
    strict_provider = str(base_extra.get("mail_provider_strict") or "").strip().lower() in {
        "1", "true", "yes", "on",
    }
    ...
    for key in ordered_keys:
        ...
        resolved_extra.setdefault("_provider_key", key)

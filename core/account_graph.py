...
PLATFORM_CREDENTIAL_TYPES: dict[str, str] = {
    ...
    "cookie": "cookie",
    "auth_cookies": "cookie",
    "oai_device_id": "identifier",
    "auth_proxy_url": "secret",
    "auth_proxy_session": "identifier",
    "api_key": "secret",
    "wos_session": "token",
    "sso": "cookie",
    "sso_rw": "cookie",
...
def _derive_validity_status(lifecycle_status: str, overview: dict[str, Any]) -> str:
    explicit = _text(overview.get("validity_status")).lower()
    if explicit in {"valid", "invalid", "unknown"}:
        return explicit
    # Lifecycle is historical/local state; a fresh remote probe is the source
    # of truth for current validity.  Only use the old lifecycle value as a
    # fallback when no explicit probe result exists.
...
def _normalize_overview_summary(
    *,
    ...
    invalid_detected_at = payload.get("invalid_detected_at")
    if isinstance(invalid_detected_at, datetime):
        payload["invalid_detected_at"] = serialize_datetime(invalid_detected_at)
    elif validity_status == "invalid" and not invalid_detected_at:
        payload["invalid_detected_at"] = payload.get("checked_at") or serialize_datetime(_utcnow())
    return payload
...
def _promote_legacy_summary_credentials(
    platform: str,
    summary: dict[str, Any],
    credentials: list[dict[str, Any]],
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    """Move credentials accidentally stored in ``legacy_extra`` into the vault table.

    ``web_access_token`` used to be omitted from ``PLATFORM_CREDENTIAL_TYPES``.
    That made new ChatGPT rows keep the correct Web token only inside the
    overview JSON while liveness checks saw the later Codex OAuth token.  Run
    this promotion inside the common persistence path so startup graph sync
    transparently repairs existing rows as well as future writes.
    """
    next_summary = dict(summary)
    legacy = _safe_dict(next_summary.get("legacy_extra"))
    if not legacy:
        return next_summary, credentials

    incoming: list[dict[str, Any]] = []
    next_legacy = dict(legacy)
    for key in PLATFORM_CREDENTIAL_TYPES:
        ...
        incoming.append(
            {
                ...
                "value": _text(value),
                "is_primary": False,
                "source": "overview.legacy_extra",
                "metadata": {},
            }
        )
        next_legacy.pop(key, None)

    if not incoming:
        return next_summary, credentials

    if next_legacy:
        next_summary["legacy_extra"] = next_legacy
    else:
        next_summary.pop("legacy_extra", None)
    return (
        next_summary,
...
def _normalize_workspace_credentials(
    existing: list[dict[str, Any]],
    incoming: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """合并 workspace 级凭证，按 (workspace_id, key) 去重，新覆盖旧。"""
    indexed: dict[tuple[str, str], dict[str, Any]] = {}
    for item in existing:
        ws_id = _safe_dict(item.get("metadata")).get("workspace_id", "")
        key = _text(item.get("key"))
        if ws_id and key:
            indexed[(ws_id, key)] = dict(item)
    for item in incoming:
        ...
        if ws_id and key:
            indexed[(ws_id, key)] = {
                ...
                "credential_type": _text(item.get("credential_type")),
                "key": key,
...
def _persist_account_graph(
    ...
    *,
    ...
    summary, platform_credentials = _promote_legacy_summary_credentials(
        platform,
        summary,
...
def sync_platform_account_graph(session: Session, model: AccountModel, account: Any) -> None:
    ...
    ws_statuses = _safe_dict(extra.get("workspace_statuses"))
    if not ws_statuses:
        ws_statuses = _safe_dict(_safe_dict(extra.get("workspace_join")).get("workspace_statuses"))
    if not ws_statuses:
        ws_statuses = _safe_dict(
            _safe_dict(_safe_dict(incoming_summary.get("legacy_extra")).get("workspace_join")).get("workspace_statuses")
        )
    if ws_statuses:
        summary["workspace_statuses"] = {
            **_safe_dict(existing_summary.get("workspace_statuses")),
            **ws_statuses,
        }
    ...
    # ★ 从 workspace_statuses 拆出每条 workspace 的凭证，存为 scope="workspace" 的 credential
    existing_ws_credentials = [item for item in current.get("credentials") or [] if item.get("scope") == "workspace"]
    incoming_ws_credentials: list[dict[str, Any]] = []
    ws_statuses_source = _safe_dict(extra.get("workspace_statuses"))
    if not ws_statuses_source:
        ws_statuses_source = _safe_dict(
            _safe_dict(extra.get("workspace_join")).get("workspace_statuses")
        )
    for ws_id, entry in ws_statuses_source.items():
        entry_creds = _safe_dict(entry.get("credentials") if isinstance(entry, dict) else {})
        if not entry_creds:
            continue
        for key, value in entry_creds.items():
            if value in (None, ""):
                continue
            incoming_ws_credentials.append({
                "scope": "workspace",
                "provider_name": platform,
                "credential_type": _infer_credential_type(key),
                "key": key,
                "value": _text(value),
                "is_primary": False,
                "source": "workspace_join",
                "metadata": {"workspace_id": ws_id},
            })
    ws_credentials = _normalize_workspace_credentials(existing_ws_credentials, incoming_ws_credentials)

    credentials = credentials + ws_credentials
...
def patch_account_graph(
    ...
    *,
    ...
    summary = _safe_dict(current.get("overview"))
    if (
        _text(current.get("validity_status")).lower() == "invalid"
        and not summary.get("invalid_detected_at")
        and summary.get("checked_at")
    ):
        # Preserve the earliest known invalid check before a new check replaces
        # checked_at. This keeps legacy invalid accounts' lifetime stable.
        summary["invalid_detected_at"] = summary["checked_at"]
    if summary_updates:

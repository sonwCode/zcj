...
def build_platform_extra(model: AccountModel, graph: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for item in _platform_credentials(graph):
        ...
        if key and value not in (None, ""):
            extra[key] = value
    # Backward compatibility for rows created before ``web_access_token`` was
    # promoted to account_credentials.  Startup sync migrates these rows, but
    # this fallback also makes hot-loaded/partially migrated databases safe.
    if isinstance(overview, dict):
        legacy = overview.get("legacy_extra")
        if isinstance(legacy, dict) and not extra.get("web_access_token"):
            web_access_token = legacy.get("web_access_token")
            if web_access_token not in (None, ""):
                extra["web_access_token"] = str(web_access_token)
    return extra

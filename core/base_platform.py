...
from core.registration import BrowserRegistrationFlow, ProtocolMailboxFlow, ProtocolOAuthFlow, ProtocolPhoneFlow, RegistrationContext, RegistrationResult
...
class AccountStatus(str, Enum):
    ...
    MANUAL_PHONE_REQUIRED  = "manual_phone_required"
...
class BasePlatform(ABC):
    ...
    def _should_require_identity_email(self) -> bool:
        return self._get_identity_provider_name() not in {"oauth_browser", "phone"}
    ...
    def _account_from_registration_result(self, result: RegistrationResult) -> Account:
        # 如果 result 直接标记了 pending 状态，优先使用
        explicit_status = getattr(result, "status", None) or (
            AccountStatus.MANUAL_PHONE_REQUIRED if getattr(result, "requires_manual_phone", False) else None
        )
        extra = dict(result.extra or {})
        if explicit_status:
            try:
                status = AccountStatus(explicit_status) if isinstance(explicit_status, str) else explicit_status
            except Exception:
                status = AccountStatus.REGISTERED
        else:
            # 检查 token 是否存在，作为"完整注册"的判断依据
            has_token = bool(
                getattr(result, "session_token", "")
                or getattr(result, "access_token", "")
                or getattr(result, "token", "")
                or getattr(result, "refresh_token", "")
                or extra.get("session_token")
                or extra.get("access_token")
                or extra.get("refresh_token")
            )
            page_type = str(extra.get("page_type") or extra.get("state") or "").strip().lower()
            pending_page_types = {
                "add_phone",
                "add-phone",
                "email_otp",
            ...
            status = (
                ...
                if has_token and page_type not in pending_page_types
        ...
        return Account(
            ...
            token=getattr(result, "session_token", "") or getattr(result, "token", ""),
            status=status,
    ...
    def register(self, email: str = None, password: str = None) -> Account:
        ...
        if getattr(identity, "identity_provider", "") == "phone":
            self.log("使用手机号协议模式注册")
            adapter = self.build_protocol_phone_adapter()
            if adapter is None:
                raise NotImplementedError(f"{self.display_name} 未实现手机号协议注册适配器")
            result = ProtocolPhoneFlow(adapter).run(ctx)
            self.raise_if_cancelled()
            return self._attach_identity_metadata(self._account_from_registration_result(result), identity)

...
def _export_selection(body: BatchExportRequest, *, platform: Optional[str] = None) -> AccountExportSelection:
    return AccountExportSelection(
        ...
        limit=max(int(body.limit or 0), 0),
    )


def _delete_exported_accounts_if_requested(body: BatchExportRequest, artifact: ExportArtifact) -> None:
    if not body.delete_after_export:
        return
    if artifact.exported_units:
        service.repository.delete_exported_units(artifact.exported_units)
        return
    for account_id in dict.fromkeys(int(item) for item in artifact.account_ids if int(item or 0) > 0):
...
def export_accounts_json(body: BatchExportRequest):
    try:
        artifact = exports_service.export_chatgpt_json(_export_selection(body))
        _delete_exported_accounts_if_requested(body, artifact)
...
def export_accounts_csv(body: BatchExportRequest):
    try:
        artifact = exports_service.export_chatgpt_csv(_export_selection(body))
        _delete_exported_accounts_if_requested(body, artifact)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return _stream_artifact(artifact)
...
def export_accounts_sub2api(body: BatchExportRequest):
    try:
        artifact = exports_service.export_chatgpt_sub2api(_export_selection(body))
        _delete_exported_accounts_if_requested(body, artifact)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return _stream_artifact(artifact)
...
def export_accounts_sub2api_agent_identity(body: BatchExportRequest):
    try:
        artifact = exports_service.export_chatgpt_agent_identity_sub2api(
            _export_selection(body, platform="chatgpt")
        )
        _delete_exported_accounts_if_requested(body, artifact)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
...
def export_accounts_cpa(body: BatchExportRequest):
    try:
        artifact = exports_service.export_chatgpt_cpa(_export_selection(body))
        _delete_exported_accounts_if_requested(body, artifact)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return _stream_artifact(artifact)
...
@router.post("/export/compact-auto")
def export_accounts_compact_auto(body: BatchExportRequest):
    try:
        artifact = exports_service.export_chatgpt_compact_auto(_export_selection(body))
        _delete_exported_accounts_if_requested(body, artifact)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return _stream_artifact(artifact)
...
def export_accounts_email_api(body: BatchExportRequest):
    try:
        artifact = exports_service.export_chatgpt_email_api_txt(_export_selection(body))
        _delete_exported_accounts_if_requested(body, artifact)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return _stream_artifact(artifact)
...
def export_accounts_cockpit(body: BatchExportRequest):
    try:
        artifact = exports_service.export_chatgpt_cockpit(_export_selection(body))
        _delete_exported_accounts_if_requested(body, artifact)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return _stream_artifact(artifact)
...
def export_accounts_kiro_go(body: BatchExportRequest):
    try:
        artifact = exports_service.export_kiro_go(_export_selection(body, platform="kiro"))
        _delete_exported_accounts_if_requested(body, artifact)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return _stream_artifact(artifact)
...
def export_accounts_any2api(body: BatchExportRequest):
    try:
        artifact = exports_service.export_any2api(_export_selection(body))
        _delete_exported_accounts_if_requested(body, artifact)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    return _stream_artifact(artifact)

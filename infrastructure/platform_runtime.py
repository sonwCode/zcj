        ...
        *,
        ...
        with Session(engine) as session:
            ...
            if isinstance(result.get("data"), dict):
                ...
                persist = result.get("_persist") if isinstance(result.get("_persist"), dict) else {}
                credential_updates = {}
                if action_ok:
                    credential_updates = {
                        key: value
                    ...
                    credential_updates.update({
                        key: value
                        for key, value in dict(persist.get("credentials") or {}).items()
                        if key in PERSISTED_ACTION_DATA_KEYS and value not in (None, "")
                    })
                summary_updates: dict[str, Any] = {}
                if action_ok:
                    summary_updates.update(dict(persist.get("summary_updates") or {}))
                ...
                if credential_updates:
                    needs_save = True
                account_email = str(persist.get("account_email") or "").strip() if action_ok else ""
                provider_accounts = list(persist.get("provider_accounts") or []) if action_ok else []
                provider_resources = list(persist.get("provider_resources") or []) if action_ok else []
                if account_email and account_email != model.email:
                    model.email = account_email
                    needs_save = True
                if provider_accounts or provider_resources or summary_updates:
                    needs_save = True
                if needs_save:
                    ...
                    patch_account_graph(
                        ...
                        provider_resources=provider_resources or None,
                    )
                    session.add(model)
                    session.commit()

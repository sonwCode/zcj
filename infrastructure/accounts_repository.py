...
class AccountsRepository:
    ...
    def list(self, query: AccountQuery) -> tuple[int, list[AccountRecord]]:
        ...
        with Session(engine) as session:
            ...
            # 状态是由账号图综合计算的，保留完整计算后再分页的语义。
            records = self._load_records(session, session.exec(statement).all())
    ...
    def select_for_export(self, selection: AccountExportSelection) -> list[AccountRecord]:
        ...
        if selection.limit > 0:
            records = records[:selection.limit]
        return records
    ...
    def delete_exported_units(self, units: list[dict]) -> int:
        grouped: dict[int, list[dict]] = {}
        for unit in units or []:
            account_id = int((unit or {}).get("account_id") or 0)
            if account_id > 0:
                grouped.setdefault(account_id, []).append(dict(unit or {}))
        if not grouped:
            return 0

        deleted = 0
        with Session(engine) as session:
            for account_id, account_units in grouped.items():
                model = session.get(AccountModel, account_id)
                if not model:
                    continue
                workspace_units = [
                    unit for unit in account_units
                    if unit.get("workspace_unit") and str(unit.get("workspace_id") or "").strip()
                ]
                if not workspace_units:
                    purge_account_graph(session, account_id)
                    session.delete(model)
                    deleted += 1
                    continue
                ...
                remove_ids = {str(unit.get("workspace_id") or "").strip() for unit in workspace_units}
                statuses = summary.get("workspace_statuses")
                if isinstance(statuses, dict):
                    for workspace_id in remove_ids:
                        statuses.pop(workspace_id, None)
                    if statuses:
                        summary["workspace_statuses"] = statuses
                    else:
                        summary.pop("workspace_statuses", None)

                legacy_join = ((summary.get("legacy_extra") or {}).get("workspace_join") or {})
                legacy_statuses = legacy_join.get("workspace_statuses")
                if isinstance(legacy_statuses, dict):
                    for workspace_id in remove_ids:
                        legacy_statuses.pop(workspace_id, None)
                    legacy_join["workspace_statuses"] = legacy_statuses
                    if isinstance(summary.get("legacy_extra"), dict):
                        summary["legacy_extra"]["workspace_join"] = legacy_join

                remaining_statuses = summary.get("workspace_statuses")
                remaining_legacy = (((summary.get("legacy_extra") or {}).get("workspace_join") or {}).get("workspace_statuses"))
                if not remaining_statuses and not remaining_legacy:
                    purge_account_graph(session, account_id)
                    session.delete(model)
                    deleted += 1
                    continue

...
const LOCAL_PROVIDER_KEYS = new Set(['local_ms_pool', 'local_gmail_pool'])
...
export default function ProviderCards({ providerType, catalog, settings, onReload, onCreateCustom }: Props) {
  ...
  for (const p of catalog) {
    const cat = providerType === 'mailbox' && LOCAL_PROVIDER_KEYS.has(p.value) ? 'local' : (p.category || 'custom')

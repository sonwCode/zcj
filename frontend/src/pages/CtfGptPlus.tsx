  ...
  const start = async () => {
    ...
    try {
      const extra: Record<string, any> = {
        ...
        // 目标数量小于并发数时也先填满并发窗口；已启动的注册/接码流程
        // 全部走完，达到目标后只停止投放新尝试。
        complete_started_attempts: Number(concurrency || 1) > 1,
        chatgpt_payment: {
          plan: "plus",
          country: payment.country,
          currency: payment.currency,
                ...
                <section className="grid gap-3 md:grid-cols-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="control-surface control-surface-compact"
                      title="用 accessToken 协议生成 pm-redirects PayPal 授权长链接，再交给所选浏览器模式继续自动填写"
                    >
                      <option value="false">{t("common.no")}</option>
                      <option value="true">{t("common.yes")}</option>
                    </select>
            ...
            >
              ...
              <div className="space-y-3 px-6 py-4">
                ...
                <textarea
                  ...
                  spellCheck={false}
                  disabled={binding}
                  placeholder="2025550101----https://relay.example.com/api/sms/recordText?key=RELAY_KEY"
                  className="control-surface control-surface-compact w-full font-mono text-xs leading-relaxed"
                />

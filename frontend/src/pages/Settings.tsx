...
import { SUB2_FREE_MODEL_SELECT_OPTIONS } from '@/lib/sub2api-models'
...
const TABS: { id: string; label: string; icon: any; sections?: any[] }[] = [
  ...
  },
  {
    ...
    sections: [{
      section: 'CPA 面板',
      desc: '注册完成后自动上传到 CPA 管理平台；请填写 CPA API 基础地址和 API Key，不要填写认证文件页面地址。',
      items: [
        { key: 'cpa_api_url', label: 'API URL', placeholder: 'https://your-cpa.example.com' },
        { key: 'cpa_api_key', label: 'Management Key', secret: true },
      ],
    }, {
      ...
      section: 'Sub2API Agent Identity',
      desc: '注册成功后上传 Agent Identity；明确失效后自动删除远端账号',
      items: [
        { key: 'sub2api_url', label: 'API URL', placeholder: 'http://172.17.0.1:8080' },
        { key: 'sub2api_admin_email', label: '管理员邮箱', placeholder: 'admin@sub2api.local' },
        { key: 'sub2api_admin_password', label: '管理员密码', secret: true },
        { key: 'sub2api_agent_identity_region', label: 'Agent Identity 代理地区', placeholder: 'CO' },
        { key: 'sub2api_proxy_id', label: 'Sub2 代理 ID', placeholder: '1' },
        {
          key: 'sub2api_default_model',
          label: '默认模型',
          options: SUB2_FREE_MODEL_SELECT_OPTIONS,
        },
        { key: 'sub2api_auto_sync', label: '注册成功自动上传', type: 'toggle' },
        { key: 'sub2api_auto_delete_invalid', label: '失效后自动删除', type: 'toggle' },
        {
          key: 'sub2api_check_interval_minutes',
          label: '有效性检查周期',
          options: [
      ...
      <div className="col-span-2 relative">
        {type === 'toggle' ? (
          <button
            type="button"
            role="switch"
            aria-checked={String(form[key] || '').toLowerCase() === 'true'}
            onClick={() => {
              const enabled = String(form[key] || '').toLowerCase() === 'true'
              setForm((current: any) => ({ ...current, [key]: enabled ? 'false' : 'true' }))
            }}
            className={`relative h-6 w-11 rounded-full border transition-colors ${String(form[key] || '').toLowerCase() === 'true' ? 'border-emerald-500 bg-emerald-500' : 'border-[var(--border)] bg-[var(--bg-pane)]'}`}
          >
            <span
              className={`absolute top-0.5 h-4.5 w-4.5 rounded-full bg-white shadow-sm transition-transform ${String(form[key] || '').toLowerCase() === 'true' ? 'translate-x-5' : 'translate-x-0.5'}`}
            />
          </button>
        ) : options ? (
          <select

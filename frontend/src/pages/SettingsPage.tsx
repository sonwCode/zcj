...
function GeneralTab({
  ...
  const oauthOptions = [
    ...
    ...(configOptions?.oauth_provider_options || []).filter(
      (o) => o.value !== "",
    ),
  ];
          ...
          <SettingRow label={t("settings.defaultOauth")}>
            <select
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  default_oauth_provider: e.target.value,
                }))
              }
              className="control-surface appearance-none"
            >
        ...
        <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] divide-y divide-[var(--border)]/50">
          <SettingRow label={t("settings.oauthEmailHint")}>
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, oauth_email_hint: e.target.value }))
              }
          ...
          <SettingRow label={t("settings.chromeProfile")}>
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, chrome_user_data_dir: e.target.value }))
              }
          ...
          <SettingRow label={t("settings.chromeCdp")}>
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, chrome_cdp_url: e.target.value }))
              }
  ...
  return (
    <div className="space-y-8">
      <SettingGroup
        ...
        desc={t("settings.versionInfo.desc")}
      >
        <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] divide-y divide-[var(--border)]/50">
              ...
              {info.latest.published_at && (
                <div className="text-xs text-[var(--text-muted)]">
                  {t("update.publishedAt", {
                    date: formatDate(info.latest.published_at, language),
                  })}
                </div>
              )}
              <Button
                size="sm"
                onClick={() =>
                  info.latest?.html_url &&
                  window.open(info.latest.html_url, "_blank")
                }
              >
      ...
      <SettingGroup title={t("settings.projectInfo")}>
        <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] divide-y divide-[var(--border)]/50">
          <InfoRow label={t("settings.projectName")} value="注册机zcj" />
          <InfoRow
            label={t("settings.techStack")}
            value="FastAPI + React + Electron"
          />
          <InfoRow label={t("settings.license")} value="AGPL-3.0" />
          <InfoRow label="Build" value="Local private build" />
          <InfoRow label="Owner" value="Private" />
        </div>
      </SettingGroup>
    </div>
  );

...
def _oauth_response_value(payload: Any, *keys: str) -> str:
    """Read a token field from standard, camelCase, or nested OAuth responses."""
    if not isinstance(payload, dict):
        return ""
    candidates = [payload]
    for container_key in ("token", "tokens", "data", "result"):
...
def normalize_oauth_token_response(payload: Any) -> dict[str, Any]:
    """Normalize an OAuth token response without inventing missing credentials."""
    return {
        "access_token": _oauth_response_value(payload, "access_token", "accessToken"),
        "refresh_token": _oauth_response_value(payload, "refresh_token", "refreshToken"),
        "id_token": _oauth_response_value(payload, "id_token", "idToken"),
        "expires_in": _to_int(_oauth_response_value(payload, "expires_in", "expiresIn")),
        "token_type": _oauth_response_value(payload, "token_type", "tokenType"),
    }

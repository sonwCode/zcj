...
本模块可读取部署方在本地提供的 HAR 模板 (``paypal_fraudnet_baseline.json``) 作为
...
# fraudnet collector iframe 使用的公开 Referer 结构
...
_BASELINE_PATH = Path(
    os.getenv("PAYPAL_FRAUDNET_BASELINE_PATH", "").strip()
    or (Path(__file__).resolve().parent / "paypal_fraudnet_baseline.json")
)
...
def _load_baseline() -> dict:
    """加载并缓存本地 baseline JSON（该运行资料不会随公开源码分发）。
...
def register_fraudnet_session(
    ...
    *,
    ...
    Args:
        ...
        ec_token: 当前会话的 EC token（如 ``EC-EXAMPLE_TOKEN``），会被

...
def normalize_cpa_api_url(value: str) -> str:
    """Normalize a CPA base URL entered in either short or full form."""
...
def upload_to_cpa(
    ...
    api_url = normalize_cpa_api_url(api_url)
    api_key = str(api_key or "").strip().strip('"').strip("'")
    if not api_url:
        return False, "CPA API URL 未配置"
    if not api_key:
        return False, "CPA Management Key 未配置"
    ...
    headers = {
        ...
        "X-API-Key": api_key,
        # CLIProxyAPI 管理接口校验专用管理头；保留兼容头以适配旧版部署。
    ...
    try:
        ...
        if response.status_code == 401:
            return False, "CPA 认证失败(401)：请填写 CPA 的 Management Key（不是普通用户 API Key）"
        if response.status_code == 404:
            return False, "CPA 接口不存在(404)：请填写 CPA 服务根地址，并确认 Management API 已开启"
...
def test_cpa_connection(api_url: str, api_token: str, proxy: str = None) -> Tuple[bool, str]:
    ...
    api_url = normalize_cpa_api_url(api_url)
    api_token = str(api_token or "").strip().strip('"').strip("'")

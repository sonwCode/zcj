...
class TokenRefreshManager:
    ...
    Token 刷新管理器
    支持两种刷新方式：
    1. OAuth Refresh Token 刷新（优先，支持 RT 轮换）
    2. Session Token 刷新（仅作为旧账号回退）
    """
    ...
    def refresh_by_session_token(
        ...
        try:
            response = None
            # The session endpoint can intermittently return a Cloudflare
            # challenge even while the same cookie and fixed exit IP are valid.
            # Retry that edge-only condition with a fresh TLS/browser session.
            for attempt in range(3):
                ...
                is_cloudflare_challenge = (
                    ...
                    and (
                        ...
                        or (
                            normalized_headers.get("server") == "cloudflare"
                            and "text/html" in normalized_headers.get("content-type", "")
                        )
                    )
                )
                if not is_cloudflare_challenge or attempt == 2:
                    break
            ...
            if response.status_code != 200:
                ...
                if result.error_code:
                    result.error_message += f" ({result.error_code})"
                logger.warning(result.error_message)
                return result

            data = response.json()
            from .oauth import normalize_oauth_token_response
            normalized = normalize_oauth_token_response(data)
            ...
            # 提取过期时间
            expires_at = None
            expires_str = data.get("expires") or data.get("expires_at") or data.get("expiresAt")
            if expires_str:
                try:
            ...
            result.refresh_token = normalized["refresh_token"] or existing_refresh_token or ""
    ...
    def refresh_by_oauth_token(
        ...
        try:
            ...
            # 提取令牌
            access_token = normalized["access_token"]
            new_refresh_token = normalized["refresh_token"] or refresh_token
            expires_in = normalized["expires_in"] or 3600
    ...
    def refresh_account(self, account: Account) -> TokenRefreshResult:
        ...
        Returns:
            TokenRefreshResult: 刷新结果
        """
        # Codex OAuth RT is the durable path. Use the web session only as a
        # fallback for legacy web-session accounts that have no RT.
        current_refresh_token = str(getattr(account, "refresh_token", "") or "").strip()
        current_session_token = str(getattr(account, "session_token", "") or "").strip()
        if current_refresh_token:
            logger.info(f"尝试使用 OAuth Refresh Token 刷新账号 {getattr(account, 'email', '')}")
            ...
            if result.success:
                return result
            logger.warning("OAuth Refresh Token 刷新失败，尝试 Session Token 回退")

        if current_session_token:
            logger.info(f"尝试使用 Session Token 刷新账号 {getattr(account, 'email', '')}")
            result = self.refresh_by_session_token(
                current_session_token,
                existing_refresh_token=current_refresh_token,
            )
            if result.success:
                return result

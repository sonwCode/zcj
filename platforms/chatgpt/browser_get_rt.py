...
def setup_oauth_state_capture(page, log: Callable[[str], None] = lambda _: None) -> None:
    ...
    def _capture_oauth_state(request):
        try:
            url = request.url
            m = re.search(r"state=([^&\s]+)", url)
            if m:
                _state_store["oauth_state"] = m.group(1)
                log(f"  [request] captured OAuth state: {m.group(1)[:20]}...")
        except Exception:
            pass

    page.on("request", _capture_oauth_state)
    log("  [request] OAuth state capture ready (no navigation interception)")

...
def resolve_runtime_browser_mode(
    mode: str,
    *,
    ...
    """Resolve a UI browser mode against the current host capabilities.

    Linux containers without ``DISPLAY`` cannot launch a headed Camoufox
    process. Only that impossible combination is adjusted; explicit headless
    and every BitBrowser mode are preserved.
    """
    normalized = str(mode or "").strip().lower() or "camoufox_headed"
    runtime_environ = os.environ if environ is None else environ
    runtime_platform = sys.platform if platform_name is None else str(platform_name)
    if normalized == "camoufox_headed" and runtime_platform.lower().startswith("linux"):
        if path_exists("/.dockerenv"):
            return "camoufox_headless"
        display = str(runtime_environ.get("DISPLAY") or "").strip()
        display_available = bool(display)
        if display.startswith(":"):
            display_number = display[1:].split(".", 1)[0]
            display_available = bool(display_number) and path_exists(

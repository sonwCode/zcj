...
from domain.proxies import ProxyBulkCreateCommand, ProxyCheckSummary, ProxyCreateCommand, ProxyRecord, ProxyUpdateCommand
...
class ProxiesService:
    ...
    def update_proxy(self, proxy_id: int, command: ProxyUpdateCommand) -> dict | None:
        item = self.repository.update(proxy_id, command)
        return self._serialize(item) if item else None
    ...
    def check_proxy(self, proxy_id: int) -> dict | None:
        ...
        refreshed = self.repository.get(proxy_id)
        return {
            "ok": bool(result.get("ok")),
            "result": result,
            "proxy": self._serialize(refreshed or item),
        }

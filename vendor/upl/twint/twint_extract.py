...
- 本脚本会提取 TWINT redirect URL；成功时该 URL 就是最终支付页。
- 默认只读取当前项目的 proxy_seeds.txt。

用法：
  1) 环境变量方式：
     PP_TOKEN="..." python twint_extract.py

  2) 文件方式：
     把 token 放到当前目录 token.txt
     python twint_extract.py

常用环境变量：
  TWINT_CONFIRM_INLINE_PM=0   # 默认按 gpthel 流程：先创建 PM，再 confirm 引用 PM
  TWINT_UPDATE_TAX_REGION=1   # TWINT 流程同步 ChatGPT/Stripe 税务地区
  TWINT_BOOTSTRAP_COUNTRY=CH  # Checkout / 首次 Stripe init 地区
  TWINT_PROMOTION_COUNTRY=VN  # checkout/update 地区
  TWINT_PROVIDER_COUNTRY=CH   # Stripe refresh / 税务 / PM / approve 地区
  TWINT_MAX_RETRY=900         # 可选；不设置时默认每批 30 轮、最多 30 批
  TWINT_PROVIDER_PER_CHECKOUT=30
  TWINT_MAX_ACCOUNT_ATTEMPTS=0   # 显式设为 0 时按代理池总容量跑
  TWINT_MAX_APPROVE_BLOCKED=900
  TWINT_WORKERS=30            # 正式流程并发数；默认每批 30 个 provider 同时跑
  TWINT_WORKERS_MAX=30
  TWINT_APPROVE_RETRY_MAX=10 # approve 阶段复用当前 provider 代理重试
  TWINT_APPROVE_STICKY=1     # approve 优先历史/当前出口，失败后切下一个 provider 出口
  TWINT_FOLLOW_REDIRECT=1
  TWINT_REQUIRE_ZERO=1        # 默认强制 0 元，不生成非 0 元链
  TWINT_DUMP_LIMIT=6000       # 抓包响应保存长度
  TWINT_PROXY_SKIP_FAILED=1   # 普通流程失败下次软跳过
  TWINT_PROXY_REMOVE_FAILED=1 # 明确的代理失败会从 proxy_seeds.txt 移除
  TWINT_PROXY_DEFAULT_SCHEME=http # 裸代理默认协议；Mars SOCKS5 可设 socks5h
  TWINT_PROXY_FAIL_COOLDOWN=180 # 失败代理冷却秒数，0 表示按旧逻辑一直跳过
  TWINT_PROXY_REMOVE_AFTER_FAILS=3 # 已复用代理健康类失败累计 3 次移除；普通代理失败 1 次移除
  TWINT_ZERO_CACHE=1          # 记录 checkout 的 0 元观察结果，供日志和排查使用
  TWINT_ZERO_CACHE_SCHEDULING=0 # 显式设为 1 才按 0 元观察结果筛选/优先调度
...
def saved_payment_value() -> str:
    return os.environ.get("TWINT_SAVED_PAYMENT_VALUE", "never").strip() or "never"
...
def default_proxy_scheme() -> str:
    raw = os.environ.get("TWINT_PROXY_DEFAULT_SCHEME", "http").strip().lower()
...
def twint_proxy_chain(proxy_seed: str) -> tuple[str, str, str]:
    ...
    promotion_proxy = proxy_for_country(proxy_seed, TWINT_PROMOTION_COUNTRY)
...
def log_twint_proxy_chain(proxy_seed: str, checkout_proxy: str, promotion_proxy: str, provider_proxy: str) -> None:
    log(
        "派生代理链: "
        f"chain={proxy_chain_key(proxy_seed)}; seed={proxy_label(proxy_seed)}; "
        f"{TWINT_BOOTSTRAP_COUNTRY} checkout={proxy_label(checkout_proxy)}; "
        f"{TWINT_PROMOTION_COUNTRY} promotion={proxy_label(promotion_proxy)}; "
        f"{TWINT_PROVIDER_COUNTRY} provider/approve={proxy_label(provider_proxy)}"
    )
...
def pre_proxy_url() -> str:
    ...
    for name in ("TWINT_PRE_PROXY", "PP_PRE_PROXY", "PP_LOCAL_PROXY"):
...
def load_proxy_seeds() -> list[str]:
    ...
    log(
        ...
        f"{TWINT_BOOTSTRAP_COUNTRY} Checkout → {TWINT_PROMOTION_COUNTRY} checkout/update → "
        f"{TWINT_PROVIDER_COUNTRY} Stripe/TWINT/approve"
    )
...
def _redact_text(text: str, limit: int | None = None) -> str:
    ...
    if limit is None:
        limit = env_int("TWINT_DUMP_LIMIT", 6000, minimum=500)
    return text[:limit]
...
def dump_http(
    ...
    if not force and not env_bool("TWINT_DUMP", False):
        return
...
def load_token() -> tuple[str, str]:
    for env_name in ("PP_TOKEN", "TWINT_TOKEN"):
...
def create_checkout(chatgpt: requests.Session, country: str) -> dict[str, str]:
    ...
    if (
        ...
        and env_bool("TWINT_COUPON_FALLBACK_PROMO_CAMPAIGN", True)
    ):
...
def update_twint_checkout_taxes(
    chatgpt: requests.Session,
    checkout: dict[str, str],
    ...
    body = {
        ...
        "billing_country": TWINT_PROVIDER_COUNTRY,
        "billing_name": billing["name"],
        "currency": currency_for_country(TWINT_PROVIDER_COUNTRY),
        "tax_id": None,
        "processor_entity": processor_entity_for_country(
            TWINT_BOOTSTRAP_COUNTRY,
            checkout.get("processor_entity") or "",
        ...
        "billing_address": {
            "line1": billing["line1"],
            "city": billing["city"],
            "country": TWINT_PROVIDER_COUNTRY,
    ...
    log(f"{TWINT_PROVIDER_COUNTRY} checkout/taxes 同步成功")
...
def stripe_create_twint_pm(stripe: requests.Session, cs_id: str, stripe_pk: str, billing: dict[str, str], ctx: dict[str, Any]) -> str:
    body: dict[str, Any] = {
        ...
        "billing_details[address][city]": billing.get("city") or "Zurich",
    ...
    if not pm_id.startswith("pm_"):
        raise RuntimeError(f"创建 TWINT PM 响应异常: {resp.text[:300]}")
    return pm_id
...
def stripe_confirm_return_url(cs_id: str, checkout: dict[str, str], stripe_hosted_url: str) -> str:
    country = normalize_country(checkout.get("billing_country") or "CH")
...
def stripe_confirm_twint(
    ...
    if env_bool("TWINT_CONFIRM_INLINE_PM", False):
        add_inline_twint_payment_method_data(body, cs_id, billing, ctx)
    else:
        body["payment_method"] = pm_id
    url = f"https://api.stripe.com/v1/payment_pages/{cs_id}/confirm"
    resp = stripe.post(url, data=body, timeout=DEFAULT_TIMEOUT)
    dump_http(resp, "twint_confirm", body, "POST", url, force=True)
    if resp.status_code >= 400:
        raise RuntimeError(f"TWINT confirm 失败 HTTP {resp.status_code}: {resp.text[:500]}")
    return resp.json() or {}
...
def is_redirect_like_url(url: str, from_action_field: bool = False) -> bool:
    ...
    if host in {"hooks.stripe.com", "payments.stripe.com"}:
        return True
    if host.endswith(".twint.ch") or host == "twint.ch":
        return True
    return any(part in text for part in ("twint", "/redirect/", "redirect_to_url", "authenticate"))
...
def approve_with_retry(
    ...
    max_retry = env_int("TWINT_APPROVE_RETRY_MAX", 10)
    parallel = env_int("TWINT_APPROVE_PARALLEL", 1)
    last_error = ""
    if max_retry <= 0:
    ...
    sticky = env_bool("TWINT_APPROVE_STICKY", True)
...
def poll_payment_page(
    ...
    cs_id = checkout["cs_id"]
    deadline = time.time() + env_int("TWINT_POLL_TIMEOUT", 45)
    params = {
    ...
    log(f"poll 结束，未发现真实 TWINT redirect/QR: {last_error}", "[WARN] ")
...
def fetch_redirect_page(stripe: requests.Session, start_url: str) -> list[str]:
    if not start_url or not env_bool("TWINT_FOLLOW_REDIRECT", True):
        return []
    current = start_url
    qr_urls: list[str] = []
...
def resolve_confirm_payload_twint(
    stripe: requests.Session,
    ...
    if not redirect_url and submission.get("state") == "requires_approval":
        ...
        log("需要 approve：TWINT 0 元场景，优先使用历史成功/当前 Provider 代理，失败后切换下一个 Provider 代理。")
        ...
        log("confirm 未返回真实 TWINT redirect，继续 poll payment_pages 做最终确认", "[WARN] ")
        redirect_url, poll_qr = poll_payment_page(stripe, checkout, stripe_pk, ctx, current_pm_id=pm_id)
        qr_urls.extend(poll_qr)
...
def run_provider_flow(
    ...
    log(
        f"{TWINT_BOOTSTRAP_COUNTRY} Bootstrap Stripe init "
    ...
    inspect_init(init_payload, f"{TWINT_BOOTSTRAP_COUNTRY} Bootstrap")
    if stop_event and stop_event.is_set():
        raise RuntimeError("任务已停止，跳过本轮")

    log(f"{TWINT_PROMOTION_COUNTRY} checkout/update: proxy={proxy_label(promotion_proxy)}")
    try:
    ...
    log(
        f"{TWINT_PROMOTION_COUNTRY} checkout/update 后通过 {TWINT_PROVIDER_COUNTRY} 刷新 Stripe: "
    ...
    if amount == 0:
        log("Promotion 后金额为 0，继续按 0 元 TWINT 流程提取最终支付 URL")

    stripe = new_session(provider_proxy)
    stripe.headers.update({"User-Agent": random_user_agent(), "Accept-Language": payment_accept_language()})

    if env_bool("TWINT_UPDATE_TAX_REGION", False):
        log(f"同步 {TWINT_PROVIDER_COUNTRY} checkout/taxes 与 Stripe tax_region...")
        tax_chatgpt = build_chatgpt_session(access_token, device_id, provider_proxy, session_token)
        update_twint_checkout_taxes(tax_chatgpt, checkout, billing)
        ...
        ctx, amount = inspect_init(init_payload, f"{TWINT_PROVIDER_COUNTRY} 税务同步")
        record_checkout_zero_result(checkout_proxy, checkout_country, amount)
        if amount != 0:
            raise RuntimeError(f"0 元优惠未生效，当前金额小单位={amount}，已停止生成非 0 元 TWINT 链")
    ...
    if env_bool("TWINT_CONFIRM_INLINE_PM", False):
        log(
            f"TWINT confirm 内联资料: {billing['name']} / "
            f"{billing['line1']} / {billing['city']} {billing['postal_code']}"
        )
    else:
        log(f"创建 PM (TWINT): {billing['country']} {billing['name']} / {billing['city']}")
        pm_id = stripe_create_twint_pm(stripe, checkout["cs_id"], stripe_pk, billing, ctx)
        log(f"PM 创建成功: {pm_id}")
        log(
            f"瑞士资料已填入 PM billing_details: {billing['name']} / "
            f"{billing['line1']} / {billing['city']} {billing['postal_code']}"
        )

    if env_bool("TWINT_UPDATE_CUSTOMER_DATA", False):
        log(
            f"提交瑞士资料填充: {billing['name']} / {billing['line1']} / "
            f"{billing['city']} {billing['postal_code']} / {billing['email']}"
        )
        stripe_update_customer_data(stripe, checkout["cs_id"], stripe_pk, ctx, billing)

    if env_bool("TWINT_CHECKOUT_SNAPSHOT", False):
    ...
    qr_urls: list[str] = []
    try:
        redirect_url, qr_urls, approve_proxy = resolve_confirm_payload_twint(
            stripe,
            confirm_payload,
            checkout,
            stripe_pk,
...
def run_once(
    ...
    checkout_country = normalize_country(os.environ.get("TWINT_CHECKOUT_COUNTRY", TWINT_BOOTSTRAP_COUNTRY))
    billing = twint_billing_profile()
    log(f"开始 TWINT 提取，第 {attempt}/{max_retry} 次")
    log(
        "组合测试: "
        f"{checkout_country} / {billing['country']} / {currency_for_country(checkout_country)} / "
        f"{payment_browser_locale()} / "
        f"{os.environ.get('TWINT_PROVIDER_COUNTRY_LABEL', TWINT_PROVIDER_COUNTRY).strip() or TWINT_PROVIDER_COUNTRY}"
    )
    if stop_event and stop_event.is_set():
        raise RuntimeError("任务已停止，跳过本轮")
    ...
    try:
        ...
        log(f"本轮代理: checkout/资格={proxy_label(checkout_proxy)}；Stripe/TWINT={proxy_label(provider_proxy)}")
...
def run_attempt(
    ...
    try:
        ...
        if is_twint_unavailable_error(error):
            log(f"第 {attempt}/{max_retry} 轮 checkout 未提供 TWINT，保留代理并继续后续组合", "[WARN] ")
            _log_context.prefix = previous_log_context
            return attempt, "", [], checkout_proxy, provider_proxy, error
...
def is_preferred_proxy(group: str, proxy: str) -> bool:
    if not group or not env_bool("TWINT_PROXY_SCORE", True):
        return False
    state = load_proxy_state().get(group, {})
...
def run_single_link_attempt(
    ...
    previous_log_context = getattr(_log_context, "prefix", "")
    _log_context.prefix = f"[TWINT {attempt}/{twint_retry}] "
    last_error = ""
    ...
    try:
        ...
        log(f"开始第 {attempt}/{twint_retry} 次提链")
        log(
            f"Step 1: 创建 ChatGPT checkout... checkout账单={checkout_country}/{checkout_currency}，"
            f"第 {attempt}/{twint_retry} 次，每次随机抽取最多 {checkout_retry} 个节点"
        )
        ...
        for checkout_index, proxy_seed in enumerate(checkout_candidates, start=1):
            if stop_event.is_set():
                return attempt, "", "任务已停止，跳过本轮", False
            _log_context.prefix = f"[TWINT {attempt}/{twint_retry}][PM={pm_country}] "
            try:
                checkout_proxy, promotion_proxy, provider_proxy = twint_proxy_chain(proxy_seed)
                log_twint_proxy_chain(proxy_seed, checkout_proxy, promotion_proxy, provider_proxy)
        ...
        if not checkout or not checkout_proxy_used:
            log(f"第 {attempt}/{twint_retry} 次提链 checkout 阶段失败", "[WARN] ")
        ...
        try:
            ...
            elif is_twint_unavailable_error(error):
                log("当前 checkout 未提供 TWINT，换下一轮 checkout 组合", "[WARN] ")
                return attempt, "", error, False
            else:
        ...
        _log_context.prefix = f"[TWINT {attempt}/{twint_retry}] "
        log(f"第 {attempt}/{twint_retry} 次提链结束，未拿到最终 URL", "[WARN] ")
...
def run_single_link_parallel_mode(
    ...
    checkout_retry = env_int("TWINT_CHECKOUT_RETRY_MAX", 5)
    provider_retry = env_int("TWINT_PROVIDER_RETRY_MAX", 3)
    twint_retry = env_int("TWINT_MAX_RETRY", 5)
    requested_workers = env_int("TWINT_WORKERS", 1)
    worker_limit = env_int("TWINT_WORKERS_MAX", requested_workers)
    ...
    if requested_workers > workers:
        log(f"TWINT并发从 {requested_workers} 限制为 {workers}", "[WARN] ")
    log(
        "开始执行 TWINT 链提取流程："
        f"checkout={checkout_country}/{checkout_currency}，PM={configured_pm_country}，locale={payment_browser_locale()}，"
        f"Checkout重试={checkout_retry}，Provider重试={provider_retry}，TWINT总重试={twint_retry}，TWINT并发={workers}。"
    )

    executor = ThreadPoolExecutor(max_workers=workers)
    futures: dict[Any, int] = {}
    try:
        for attempt in range(1, twint_retry + 1):
            futures[
                executor.submit(
        ...
        for future in as_completed(futures):
            try:
                ...
                error = str(exc)
                approve_blocked = False
                log(f"第 {attempt}/{twint_retry} 次提链异常: {error[:300]}", "[WARN] ")
            if redirect_url:
                stop_event.set()
                for pending in futures:
                    pending.cancel()
                print("\n===== 结果 =====")
                print(f"TWINT 最终支付 URL:\n{redirect_url}")
                return 0
...
def run_single_link_mode(
    ...
    twint_workers = env_int("TWINT_WORKERS", 1)
    if twint_workers > 1:
        log(f"TWINT 链路固定并发=1，忽略 TWINT_WORKERS={twint_workers}", "[WARN] ")
    ...
    log(
        "开始执行 TWINT 链提取流程："
        f"checkout={checkout_country}/{checkout_currency}，PM={configured_pm_country}，locale={payment_browser_locale()}，"
        f"Checkout重试={checkout_retry}，Provider重试={provider_retry}，TWINT总重试={twint_retry}。"
    )
    ...
    for attempt in range(1, twint_retry + 1):
        ...
        if not checkout or not checkout_proxy_used:
            log(f"第 {attempt}/{twint_retry} 次提链 checkout 阶段失败，换下一次提链", "[WARN] ")
            continue
        ...
        try:
            ...
            if is_checkout_not_active_error(error):
                ...
                log("当前 checkout 未提供 TWINT，换下一次提链", "[WARN] ")
            else:
                record_failure_by_stage(error, checkout_proxy_used, provider_proxy, promotion_proxy)
                log(f"Provider 失败: {error[:220]}", "[WARN] ")
                if is_approve_failure_error(error) and "approve blocked" in error:
...
def main() -> int:
    ...
    flow_mode = os.environ.get("TWINT_FLOW_MODE", "single").strip().lower() or "single"
    if flow_mode != "single":
        log(f"TWINT_FLOW_MODE={flow_mode} 已收敛为 strict single seed 链路", "[WARN] ")

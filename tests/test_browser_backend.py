...
def test_runtime_browser_mode_uses_headless_without_linux_display():
    assert bbe.resolve_runtime_browser_mode(
        "camoufox_headed",
        environ={},
        platform_name="linux",
    ) == "camoufox_headless"
...
def test_runtime_browser_mode_keeps_headed_with_linux_display():
    assert bbe.resolve_runtime_browser_mode(
        "camoufox_headed",
        environ={"DISPLAY": ":99"},
        platform_name="linux",
        path_exists=lambda path: path == "/tmp/.X11-unix/X99",
    ) == "camoufox_headed"


def test_runtime_browser_mode_uses_headless_for_stale_linux_display():
    assert bbe.resolve_runtime_browser_mode(
        "camoufox_headed",
        environ={"DISPLAY": ":99"},
        platform_name="linux",
...
def test_runtime_browser_mode_uses_headless_in_linux_container():
    existing_paths = {"/.dockerenv", "/tmp/.X11-unix/X99"}
    assert bbe.resolve_runtime_browser_mode(
        "camoufox_headed",
        environ={"DISPLAY": ":99"},
        platform_name="linux",
...
@pytest.mark.parametrize(
    "mode",
    ["camoufox_headless", "bitbrowser_headed", "bitbrowser_hidden", "bitbrowser_headless"],
)
def test_runtime_browser_mode_preserves_explicit_modes(mode):
    assert bbe.resolve_runtime_browser_mode(
        mode,
        environ={},
        platform_name="linux",

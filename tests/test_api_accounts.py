...
def test_export_sub2api_rejects_workspace_record_with_free_token():
    access_token = _make_jwt({
        "exp": 1777166030,
        "https://api.openai.com/auth": {
    ...
    try:
        service.export_chatgpt_sub2api(AccountExportSelection(platform="chatgpt", select_all=True))
    except ValueError as exc:
        message = str(exc)
    else:
        raise AssertionError("free personal token with workspace_id should not export as Sub2Api")

    assert "workspace_id=workspac" in message
    assert "access_token" in message
    assert "free" in message


def test_export_chatgpt_json_includes_token_plan_type():
    access_token = _make_jwt({
        "exp": 1777166030,
        "https://api.openai.com/auth": {
            "chatgpt_account_id": "acct-json",
            "chatgpt_plan_type": "free",
        },
    })
    repository = AccountsRepository()
    ...
    artifact = service.export_chatgpt_json(AccountExportSelection(platform="chatgpt", select_all=True))
    payload = json.loads(artifact.content)

    assert payload[0]["token_plan_type"] == "free"


def test_export_chatgpt_json_expands_workspace_status_exports(tmp_path):
    workspace_a = tmp_path / "workspace-a.json"
    workspace_b = tmp_path / "workspace-b.json"
        ...
        json.dumps(
            {
                ...
                "last_refresh": "2026-07-04T10:00:00Z",
                "expired": "2026-07-14T10:00:00Z",
            }
        ),
        encoding="utf-8",
    )
        ...
        json.dumps(
            {
                ...
                "last_refresh": "2026-07-04T10:01:00Z",
                "expired": "2026-07-14T10:01:00Z",
            }
        ),
        encoding="utf-8",
    )
    ...
    repository.create(
        AccountCreateCommand(
            ...
            email="multi-workspace@test.com",
            password="TestPass123!",
            credentials={"access_token": "base-access", "workspace_id": "workspace-a"},
            overview={
                "workspace_statuses": {
                    "workspace-a": {"status": "export_ok", "json_path": str(workspace_a)},
                    "workspace-b": {"status": "export_ok", "json_path": str(workspace_b)},
                }
            },
        )
    )
    ...
    assert [item["workspace_id"] for item in payload] == ["workspace-a", "workspace-b"]
    assert [item["access_token"] for item in payload] == ["access-a", "access-b"]
    assert [item["account_id"] for item in payload] == ["account-a", "account-b"]


def test_export_chatgpt_cpa_expands_workspace_status_exports(tmp_path):
    workspace_a = tmp_path / "workspace-a.json"
    workspace_b = tmp_path / "workspace-b.json"
    workspace_a.write_text(json.dumps({"email": "cpa-workspace@test.com", "access_token": "access-a", "account_id": "workspace-a", "chatgpt_account_id": "workspace-a", "chatgpt_plan_type": "k12"}), encoding="utf-8")
    workspace_b.write_text(json.dumps({"email": "cpa-workspace@test.com", "access_token": "access-b", "account_id": "workspace-b", "chatgpt_account_id": "workspace-b", "chatgpt_plan_type": "k12"}), encoding="utf-8")
    repository = AccountsRepository()
    created = repository.create(
        AccountCreateCommand(
            platform="chatgpt",
            email="cpa-workspace@test.com",
            password="TestPass123!",
            overview={
                "workspace_statuses": {
                    "workspace-a": {"status": "export_ok", "json_path": str(workspace_a)},
    ...
    artifact = service.export_chatgpt_cpa(AccountExportSelection(platform="chatgpt", ids=[created.id]))

    assert artifact.media_type == "application/json"
    payloads = json.loads(artifact.content)
    assert [item["workspace_id"] for item in payloads] == ["workspace-a", "workspace-b"]
    assert [item["access_token"] for item in payloads] == ["access-a", "access-b"]


def test_export_chatgpt_compact_auto_adds_private_workspace_aliases(tmp_path):
    workspace_a = tmp_path / "workspace-a.json"
    workspace_a.write_text(
        json.dumps(
            {
                "email": "compact-workspace@test.com",
                "access_token": "access-a",
                "session_token": "session-a",
                "id_token": "fake.synthetic.id-token",
                "id_token_synthetic": True,
                "account_id": "workspace-a",
                "chatgpt_account_id": "workspace-a",
                "chatgpt_plan_type": "k12",
                "expired": "2026-07-15T08:23:00Z",
            }
        ),
        encoding="utf-8",
    )
    ...
    artifact = service.export_chatgpt_compact_auto(AccountExportSelection(platform="chatgpt", ids=[created.id]))

    assert artifact.media_type == "application/json"
    payload = json.loads(artifact.content)
    assert payload["access_token"] == "access-a"
    assert payload["id_token"] == "access-a"
    assert "id_token_synthetic" not in payload
    assert payload["session_token"] == "session-a"
    assert payload["account_id"] == "workspace-a"
    assert payload["workspace_id"] == "workspace-a"
    assert payload["plan_type"] == "k12"
    assert payload["private"] is True
    assert payload["is_private"] is True
    assert payload["expires_at"] == "2026-07-15T08:23:00Z"


def test_export_chatgpt_sub2api_expands_workspace_status_exports(tmp_path):
    workspace_a = tmp_path / "workspace-a.json"
    workspace_b = tmp_path / "workspace-b.json"
    workspace_a.write_text(
        json.dumps({"account_id": "account-a", "chatgpt_plan_type": "k12", "access_token": "access-a"}),
        encoding="utf-8",
    )
    workspace_b.write_text(
        json.dumps({"account_id": "account-b", "chatgpt_plan_type": "k12", "access_token": "access-b"}),
        encoding="utf-8",
    )
    repository = AccountsRepository()
    created = repository.create(
    ...
    artifact = service.export_chatgpt_sub2api(AccountExportSelection(platform="chatgpt", ids=[created.id]))
    payload = json.loads(artifact.content)

    assert [item["credentials"]["organization_id"] for item in payload["accounts"]] == ["workspace-a", "workspace-b"]
    assert [item["credentials"]["access_token"] for item in payload["accounts"]] == ["access-a", "access-b"]


def test_export_json_delete_after_export_removes_only_exported_workspace(tmp_path):
    workspace_a = tmp_path / "workspace-a.json"
    workspace_b = tmp_path / "workspace-b.json"
    workspace_a.write_text(
        json.dumps({"account_id": "account-a", "chatgpt_plan_type": "k12", "access_token": "access-a"}),
    ...
    service = AccountExportsService(repository)
    artifact = service.export_chatgpt_json(
        AccountExportSelection(platform="chatgpt", ids=[created.id], limit=1)
    )
    repository.delete_exported_units(artifact.exported_units)

    assert [item["workspace_id"] for item in json.loads(artifact.content)] == ["workspace-a"]
    remaining = repository.get(created.id)
    assert remaining is not None
    assert sorted((remaining.overview.get("workspace_statuses") or {}).keys()) == ["workspace-b"]
...
def test_parse_phone_bind_lines_accepts_multiple_phone_api_pairs():
    entries = parse_phone_bind_lines(
        "\n".join(
            [
                "2025550101----https://relay.example.com/api/sms/recordText?key=RELAY_KEY_1",
                "+12025550102 ---- https://relay.example.com/api/sms/recordText?key=RELAY_KEY_2",
            ]
        )
    )

    assert [entry.phone for entry in entries] == ["+12025550101", "+12025550102"]
    assert entries[0].sms_api.endswith("key=RELAY_KEY_1")
...
def test_phone_binding_marks_selected_accounts_and_reports_phone_usage():
    ...
    result = service.bind(
        ...
        phone_lines="2025550101----https://relay.example.com/api/sms/recordText?key=RELAY_KEY_1",
    )

    assert result["total"] == 2
    assert result["success_count"] == 2
    assert result["failure_count"] == 0
    assert result["phones"][0]["phone"] == "+12025550101"
    assert result["phones"][0]["used"] == 2
    assert calls == [
        ("bind1@test.com", "+12025550101"),
        ("bind2@test.com", "+12025550101"),
    ]
    updated = repository.get(first.id)
    assert updated is not None
    assert updated.overview["phone_binding"]["status"] == "bound"
    assert updated.overview["phone_binding"]["phone"] == "+12025550101"
...
def test_phone_binding_accepts_uncapped_concurrency():
    ...
    result = service.bind(
        ...
        phone_lines="\n".join(
            [
                "2025550101----https://relay.example.com/api/sms/recordText?key=RELAY_KEY_1",
                "2025550102----https://relay.example.com/api/sms/recordText?key=RELAY_KEY_2",
            ]
        ),
        concurrency=99,
    )
    ...
    class FakePhoneBindingService:
        def bind(self, **kwargs):
            return {
                ...
                "phones": [{"phone": "+12025550101", "used": 1, "success": 1, "failed": 0}],
                "results": [],
            }

...
def test_get_rt_uses_supplied_phone_callback(monkeypatch):
    ...
    monkeypatch.setattr(
        protocol_phone_module.ChatGPTProtocolEmailThenPhoneWorker,
        "run_for_account",
        lambda self, **kwargs: (_ for _ in ()).throw(RuntimeError("protocol fallback test")),
    )
...
def test_get_rt_protocol_success_skips_browser(monkeypatch):
    monkeypatch.setattr(
        ChatGPTPlatform,
        "_build_get_rt_mailbox_otp_callback",
        lambda self, account, log_fn, proxy: (lambda: "123456", ""),
    ...
    monkeypatch.setattr(
        ...
        lambda self, **kwargs: {
            ...
            "id_token": "protocol-id",
        },
    )
    monkeypatch.setattr(
        browser_register_module.ChatGPTBrowserRegister,
        "_open_browser",
        lambda self, launch_opts: (_ for _ in ()).throw(AssertionError("browser should not open")),
    )
    ...
    assert result["ok"] is True
    assert result["data"]["refresh_token"] == "protocol-refresh"
